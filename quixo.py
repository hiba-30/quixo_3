...
class Quixo:
    ...
    def déplacer_pion(self, pion, origine, direction):
        ...
        """
        Plateau().insérer_un_cube(pion, origine, direction)
    ...
    def choisir_un_coup(self):
        ...
        QuixoError().position_non_valide(origine)
        origine = [int(x) for x in origine.split(',')]
        dirt = "('haut', 'bas', 'gauche', 'droite')"
        phrase = f"Quelle direction voulez-vous insérer? {dirt} : "
        direction = input(phrase)
